...
@Table(name = "member", uniqueConstraints = {
		@UniqueConstraint(name = "uk_member_login_id", columnNames = { "loginId" }) }) // 데이터베이스에서 중복이 가능하지 않도록 관리할 수
																						// 있음!
...
public class Member {
	...
	@Column(nullable = false, length = 30, updatable = false) // 꼭 필수로 입력해야하는 정보이기 때문에 null 값을 허용하지 않고 30자까지 인정, 업데이트도
																// 가능하지 않도록 함!
